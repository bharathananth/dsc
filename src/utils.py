...
class Logger:
    ...
    def error(self, msg=None, q=True):
        ...
        if q:
            sys.stderr.write(
                start +
                f"\033[1;33mERROR: {self.emphasize(msg, 33)}\033[0m\n" + end)
            sys.exit(1)
        else:
            sys.stderr.write(
                start +
                f"\033[1;35mWARNING: {self.emphasize(msg, 35)}\033[0m\n" + end)
    ...
    def log(self, msg=None, flush=False, debug=False):
        ...
        if isinstance(msg, list):
            ...
            msg = str(msg)
        start = "{0:{width}}".format('\r', width=self.__width_cache +
                                     10) + "\r" if flush else ''
        end = '' if flush else '\n'
        ...
        if debug:
            sys.stderr.write(
                start + f"\033[1;34mDEBUG: {self.emphasize(msg, 34)}\033[0m" +
                end)
        else:
            sys.stderr.write(
                start + f"\033[1;32mINFO: {self.emphasize(msg, 32)}\033[0m" +
                end)
    ...
    def emphasize(msg, level_color):
        if msg is None:
            return msg
        return re.sub(r'``([^`]*)``',
                      f'\033[0m\033[1;4m\\1\033[0m\033[1;{level_color}m',
                      str(msg))
...
def pairwise_list(*args):
    if len(set(len(x) for x in args)) > 1:
        raise ValueError(
            "Cannot perform pairwise operation because input vectors are not of equal lengths."
        )
...
def get_slice(value, all_tuple=True, mismatch_quit=True, non_negative=True):
    ...
    try:
        ...
        if mismatch_quit:
            raise AttributeError(
                f'Cannot obtain slice from input string {value}. [{e}].')
        else:
            return value, None
    name = value.split('[')[0]
    idxs = []
    for item in slicearg.split(','):
        ...
        if ':' in item:
            ...
            idxs.extend([
                x - 1 for x in range(slice_obj.start or 0, slice_obj.stop
                                     or -1, slice_obj.step or 1)
            ])
        else:
            idxs.append(int(item) - 1)
    if any([i < 0 for i in idxs]) and non_negative:
        raise ValueError(
            "Invalid slice ``{}``. Indices should be positive integers.".
            format(value))
...
def expand_slice(line):
    ...
    for m in re.finditer(pattern, line):
        ...
        if len(sliced_text[1]) == 1:
            continue
        text = '({})'.format(','.join(
            ['{}[{}]'.format(sliced_text[0], x + 1) for x in sliced_text[1]]))
...
def find_parens(s, lenient=True, start='(', end=')'):
    ...
    for i, c in enumerate(s):
        if c == start:
            ...
            if len(pstack) == 0:
                if not lenient:
                    raise IndexError(
                        f"No matching closing ``{end}`` at position {i+1}, near ``{s}``"
                    )
                else:
                    break
            toret[pstack.pop()] = i
    if len(pstack) > 0:
        raise IndexError(
            f"No matching opening ``{start}`` at position {pstack.pop() + 1}, near ``{s}``"
        )
    return toret
